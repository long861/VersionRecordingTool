...
module.exports.configureSocketIO = function (app, mongooseConn) {
  ...
  let server = require('./socket.io')(app, mongooseConn);
...
module.exports.initInstanceId = function (app) {
  let instanceId = parseInt((process.env.NODE_APP_INSTANCE || '-1'), 10);
  logger.info('================================================================');
...
module.exports.init = function (mongooseConn) {
  ...
  app.all('*', function (req, res, next) {
    res.header("Access-Control-Allow-Origin", '*');//TODO:后续针对性开放，暂时放开保障爱心互助金运行@XT@2021年01月14日
    res.header("Access-Control-Allow-Headers", "Content-Type,Content-Length, Authorization, Accept,X-Requested-With");
    res.header("Access-Control-Allow-Methods", "PUT,POST,GET,DELETE,OPTIONS");
    res.header("Access-Control-Allow-Credentials", "true");
    res.header("X-Powered-By", ' 3.2.1')

    if (req.method === "OPTIONS") return res.sendStatus(200);/*让options请求快速返回*/
    next();
  });
